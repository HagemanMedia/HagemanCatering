...
.wc-back-portal-button {
    ...
    font-weight: bold;
    border-radius: 4px;
    text-decoration: none !important; /* niet underlined */
    line-height: 32px; /* precies gelijk aan hoogte */
    cursor: pointer;
    border: none;
}
...
/* Mobiele optimalisaties CSS */
@media (max-width: 768px) {
    /* Verberg “Deze week” knop */
    .wc-today-button {
        display: none !important;
    }

    /* Datum helemaal bovenaan */
    ...
    /* Knopjes daaronder links uitlijnen */
    ...
    /* 1) Maatwerk, gebruiker en turflijst naast elkaar */
    .wc-nav-elements-left {
        flex: none;
    }
    ...
    /* 3) Refresh en home als kleine icoontjes naast pijltjes */
    .wc-home-button,
    .wc-refresh-button,
    .wc-arrow-button {
        width: 32px;
...
.birthday-item   { border-left: 4px solid #FF8C00; }
.status-birthday { background-color: #FF8C00 !important; }



.wc-add-maatwerk-day-button {
...
.wc-add-maatwerk-day-button:hover {
    background-color: #388E3C;
}

/* Zoekveld als knop-achtige input */
#wc-search-input {
...
/* Placeholder kleur licht(er) zodat leesbaar is */
...
/* Datum picker styled als de andere navigatieknoppen, zonder typen */
#wc-week-date-picker {
    background-color: #009541;
    color: #fff;
    border: none;
...
/* Zorg dat de kalenderindicator (pijltje) zichtbaar én wit is */
#wc-week-date-picker::-webkit-calendar-picker-indicator {
    filter: invert(1);
}

/* Focus styling consistent houden */
#wc-week-date-picker:focus {
    outline: 2px solid rgba(255,255,255,0.5);
    outline-offset: 0;
}
...
.wc-modal-detail-section details summary { cursor: pointer; }
.wc-modal-detail-section ul { margin: 4px 0 0; padding-left: 18px; }
...
$output .= '<div class="wc-agenda-navigation" style="display:flex;align-items:center;gap:8px;flex-wrap:nowrap;">';

// Groepeer Maatwerk, Klant toevoegen en Turflijst knoppen aan de linkerkant
$output .= '<div class="wc-nav-elements-left" style="display:flex;align-items:center;gap:6px;">';

if (in_array('administrator', $user_roles)) { 
    // Nieuwe brede knop "Terug naar het portaal"
    $output .= '<a href="https://banquetingportaal.nl/alg/wp-admin/" target="_blank" class="wc-nav-button wc-back-portal-button" style="display:flex;align-items:center;justify-content:center;height:32px;padding:0 10px;background-color:#2271b1;color:#fff;font-weight:bold;border-radius:4px;text-decoration:none;line-height:32px;">
    ...
    Terug naar het portaal</a>';
}

if (in_array('administrator', $user_roles)) { 
    $output .= '<button class="wc-nav-button wc-new-maatwerk-button icon-only">';
    $output .= '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path fill="currentColor" d="M96 32l0 32L48 64C21.5 64 0 85.5 0 112l0 48 448 0 0-48c0-26.5-21.5-48-48-48l-48 0 0-32c0-17.7-14.3-32-32-32s-32 14.3-32 32l0 32L160 64l0-32c0-17.7-14.3-32-32-32S96 14.3 96 32zM448 192L0 192 0 464c0 26.5 21.5 48 48 48l352 0c26.5 0 48-21.5 48-48l0-272zM224 248c13.3 0 24 10.7 24 24l0 56 56 0c13.3 0 24 10.7 24 24s-10.7 24-24 24l-56 0 0 56c0 13.3-10.7 24-24 24s-24-10.7-24-24l0-56-56 0c-13.3 0-24-10.7-24-24s10.7-24 24-24l56 0 0-56c0-13.3 10.7-24 24-24z"/></svg>';
    $output .= '</button>'; 

    $output .= '<button class="wc-nav-button wc-add-customer-button icon-only">';
    $output .= '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512"><path fill="currentColor" d="M96 128a128 128 0 1 1 256 0A128 128 0 1 1 96 128zM0 482.3C0 383.8 79.8 304 178.3 304l91.4 0C368.2 304 448 383.8 448 482.3c0 16.4-13.3 29.7-29.7 29.7L29.7 512C13.3 512 0 498.7 0 482.3zM504 312l0-64-64 0c-13.3 0-24-10.7-24-24s10.7-24 24-24l64 0 0-64c0-13.3 10.7-24 24-24s24 10.7 24 24l0 64 64 0c13.3 0 24 10.7 24 24s-10.7 24-24 24l-64 0 0 64c0 13.3-10.7 24-24 24s-24-10.7-24-24z"/></svg>';
    $output .= '</button>'; 
}

$output .= '<button class="wc-nav-button wc-turflijst-button icon-only">';
$output .= '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path fill="currentColor" d="M152.1 38.2c9.9 8.9 10.7 24 1.8 33.9l-72 80c-4.4 4.9-10.6 7.8-17.2 7.9s-12.9-2.4-17.6-7L7 113C-2.3 103.6-2.3 88.4 7 79s24.6-9.4 33.9 0l22.1 22.1 55.1-61.2c8.9-9.9 24-10.7 33.9-1.8zm0 160c9.9 8.9 10.7 24 1.8 33.9l-72 80c-4.4 4.9-10.6 7.8-17.2 7.9s-12.9-2.4-17.6-7L7 273c-9.4-9.4-9.4-24.6 0-33.9s24.6-9.4 33.9 0l22.1 22.1 55.1-61.2c8.9-9.9 24-10.7 33.9-1.8zM224 96c0-17.7 14.3-32 32-32l224 0c17.7 0 32 14.3 32 32s-14.3 32-32 32l-224 0c-17.7 0-32-14.3-32-32zm0 160c0-17.7 14.3-32 32-32l224 0c17.7 0 32 14.3 32 32s-14.3 32-32 32l-224 0c-17.7 0-32-14.3-32-32zM160 416c0-17.7 14.3-32 32-32l288 0c17.7 0 32 14.3 32 32s-14.3 32-32 32l-288 0c-17.7 0-32-14.3-32-32zM48 368a48 48 0 1 1 0 96 48 48 0 1 1 0-96z"/></svg>';
$output .= '</button>'; 

$output .= '</div>'; // Sluit wc-nav-elements-left

// Zoekbalk in lijn
$output .= '<div style="flex:1;display:flex;justify-content:center;align-items:center;">';
$output .= '<input type="text" id="wc-search-input" placeholder="zoekbalk werkt nog niet!" aria-label="Zoeken" style="width:300px;max-width:100%;padding:6px 8px;border:1px solid #ccc;border-radius:4px;" />';
$output .= '</div>';

// Rechts: geannuleerd toggle + refresh + weekselector
$output .= '<div class="wc-nav-elements-right" style="display:flex;align-items:center;gap:6px;">';

$output .= '<button id="wc-hide-cancelled" class="wc-nav-button" type="button" title="Verberg of toon geannuleerde orders" aria-pressed="false">Verberg geannuleerd</button>';
$output .= '<style>@media (max-width:768px){#wc-hide-cancelled{display:none!important}}</style>';

$output .= '<button class="wc-nav-button wc-refresh-button" aria-label="Verversen" title="Verversen" type="button" onclick="location.reload();" style="display:flex;align-items:center;justify-content:center;padding:0 10px;height:32px;">
...
// Week selector blok
$output .= '<div style="display:flex;gap:5px;align-items:center;">';
$output .= '<button class="wc-nav-button wc-today-button">Deze week</button>';
$output .= '<button class="wc-nav-button wc-arrow-button" data-direction="-1">&lt;</button>';
$output .= '<input type="date" id="wc-week-date-picker" value="' . esc_attr(date('Y-m-d', $week_start)) . '" aria-label="Kies week" autocomplete="off" />';
$output .= '<button class="wc-nav-button wc-arrow-button" data-direction="1">&gt;</button>';
$output .= '</div>'; // einde week selector
...
// ✅ ONDERAAN ELK MAATWERK-KAARTJE: "Turflijst bekijken/wijzigen"
if ($item['type'] === 'maatwerk') {
    global $wpdb;

    // Bestaat er een turflijst voor dit event?
    $turftable      = $wpdb->prefix . 'custom_form_entries';
    $has_turflijst  = (int) $wpdb->get_var( $wpdb->prepare("SELECT COUNT(*) FROM {$turftable} WHERE event_id = %d", $item['order_id']) ) > 0;

    if ($has_turflijst) {
        // Zichtbaarheid: admin altijd; medewerkers tot 8 uur na eindtijd
        $now_ts = current_time('timestamp');

        // Eindtijd bepalen (val terug op laatste bekende tijd of 23:59)
        $event_date_y_m_d = date('Y-m-d', $day_ts);
        $end_time_str = '';
        if (!empty($item['eindtijd'])) {
            // Strip eventuele extra tekens; verwacht "HH:MM"
            $end_time_str = preg_replace('/[^0-9:]/', '', $item['eindtijd']);
        } elseif (!empty($all_times)) {
            // Neem laatste tijd uit eerder verzamelde tijden
            $tmp = $all_times; // kopie zodat end() veilig is
            $end_time_str = end($tmp);
        }
        if (empty($end_time_str)) {
            $end_time_str = '23:59';
        }

        $event_end_ts = strtotime($event_date_y_m_d . ' ' . $end_time_str);
        if (!$event_end_ts) {
            $event_end_ts = strtotime($event_date_y_m_d . ' 23:59:59');
        }
        $visible_until_ts = $event_end_ts + (8 * HOUR_IN_SECONDS);

        $is_admin        = current_user_can('manage_options');
        $within_8_hours  = ($now_ts <= $visible_until_ts);

        if ($is_admin || $within_8_hours) {
            // Link naar editor (opent in nieuw venster)
            $turflijst_url = add_query_arg(
                array('turflijst_edit' => (int)$item['order_id']),
                home_url('/alg/turf/')
            );

            // Tekst: "✅ Turflijst bekijken/wijzigen"
            // Vereiste styling: "Turflijst bekijken" wit & onderstreept; klikt door naar editor
            $output .= '<div class="wc-info-text" style="margin-top:8px;">'
                     . '✅ <a href="' . esc_url($turflijst_url) . '" target="_blank" rel="noopener"'
                     . ' style="color:#fff; text-decoration:underline;">Turflijst bekijken</a> / wijzigen'
                     . '</div>';
        }
    }
}
...
function getFilterWeekDate() {
    const params = new URLSearchParams(window.location.search);
    const fw = params.get("filter_week");
    if (fw) {
        const d = new Date(fw);
        if (!isNaN(d)) return d;
    }
...
if (datePicker) {
    // Blokkeer handmatig typen, maar laat klik (calendar popup) werken
...
// Bij mobiel: standaard naar huidige dag redirecten als er geen filter_day in de URL staat
function mobileDefaultToToday() {
    ...
    if (!params.get("filter_day")) {
        ...
        // ✅ MOBIEL: ook meteen filter_week (maandag van deze week) zetten
        const mon = new Date(today);
        const day = (mon.getDay() + 6) % 7;
        mon.setDate(mon.getDate() - day);
        params.set("filter_week", formatDateYYYYMMDD(mon));
...
function initWeekNavigationButtons() {
    // Bij mobiel meteen zorgen dat we op de dagview staan
    ...
    if (todayBtn) {
        todayBtn.addEventListener("click", function(e) {
            ...
            if (isMobileView()) {
                ...
                // ✅ MOBIEL: ook filter_week zetten (maandag van de huidige week)
                const mon = new Date(today);
                const day = (mon.getDay() + 6) % 7;
                mon.setDate(mon.getDate() - day);
                url.searchParams.set("filter_week", formatDateYYYYMMDD(mon));
            } else {
    ...
    document.querySelectorAll(".wc-arrow-button").forEach(btn => {
        btn.addEventListener("click", function(e) {
            e.preventDefault();
            const params = new URLSearchParams(window.location.search);
            if (isMobileView()) {
                // Per dag navigeren
                const fd = params.get("filter_day");
                let currentDate = fd ? new Date(fd) : new Date();
                ...
                // ✅ MOBIEL: ook filter_week bijwerken (maandag van deze dag)
                ...
                // Per week navigeren
                const fw = params.get("filter_week");
                let reference = fw ? new Date(fw) : new Date();
                // Normalize naar maandag
    ...
    if (datePicker) {
        datePicker.addEventListener("change", function () {
            ...
            if (isMobileView()) {
                ...
                // ✅ MOBIEL: ook filter_week bijwerken (maandag van gekozen datum)
...
function initOrderTypeFilters() {
    const filterBanquetingBtn = document.getElementById("wc-filter-wc");
    const hideCancelledBtn = document.getElementById("wc-hide-cancelled");

    if (filterBanquetingBtn) {
        filterBanquetingBtn.dataset.active = filterBanquetingBtn.dataset.active || "0";
        filterBanquetingBtn.textContent = filterBanquetingBtn.dataset.active === "1" ? "Toon alle orders" : "Toon alleen banqueting";
        filterBanquetingBtn.addEventListener("click", function () {
    ...
    if (hideCancelledBtn) {
        // Standaard: geannuleerde (en afgewezen) items VERBORGEN
        hideCancelledBtn.dataset.active = "1";
        hideCancelledBtn.textContent = "Toon geannuleerd";
        hideCancelledBtn.setAttribute("aria-pressed", "true");

        // Initieel verbergen bij laden
        document.querySelectorAll(".wc-weekagenda-item-google").forEach(item => {
            ...
            if (isCancelled) item.style.display = "none";
        });
...
function initAgendaNavigation() {
    ...
    if (todayButton) {
        // zorg dat er geen dubbele listener komt
        function handleTodayClick(e) {
            e.preventDefault();
            const todayDate = this.dataset.weekStart; // bevat huidige dag
            const newUrl = new URL(window.location.href);
            // ✅ Altijd expliciet met strings voor de param-namen
